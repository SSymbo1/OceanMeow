...
import { onMounted, Ref, ref, h, watch, nextTick } from 'vue'
import { menu } from '@/assets/json/home_menu.json'
import { HomeOutlined, InboxOutlined, SettingOutlined, LinkOutlined } from '@ant-design/icons-vue'
import { useRouter } from 'vue-router';
import { useAccountStore } from '@/pinia/store/account';
import SelectInstallPathModal from '@/components/component/SelectInstallPathModal.vue';
import SelectAccountModal from '@/components/component/SelectAccountModal.vue';
...
const menuItems: Ref<Object[]> = ref([])
...
menuItems.value = menu.map((item, index) => ({
    key: item.key,
    label: item.label,
    title: item.name,
    route: item.route,
...
const appBaseDataInit = async () => {
    ...
    updateAccountInfo()
    await window.electronAPI.collectLibraryData(useSteamStore().steam.installPath)
    window.electronAPI.collectScreenshotData(useSteamStore().steam.installPath)
}
...
<template>
    <a-layout class="h-full flxe-1">
        <a-layout-sider class="min-h-full" v-model:collapsed="collapsed" collapsible>
            <div class="flex items-center p-4 transition-all mt-2.5">
                <div class="relative w-12 rounded-full overflow-hidden border-2 flex-shrink-0">
                    <a-avatar :src="`load://${useSteamStore().steam.installPath.replace(/\\/g, '/')}${accountAvator}`"
                        :size="43" />
                </div>
                <div class="ml-3 text-white font-medium overflow-hidden transition-all duration-300 ease-in-out"
