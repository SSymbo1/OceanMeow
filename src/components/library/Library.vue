...
import { FieldTimeOutlined, TagOutlined, KeyOutlined, SettingOutlined, DeleteOutlined } from '@ant-design/icons-vue';
...
const handleGridMenuClick = (e: Event) => {
    console.log(e)
}
    ...
    <a-layout class="h-full w-full overflow-hidden">
        <a-layout-content class="flex flex-col h-full">
            <div class="h-15 px-3 py-1 flex flex-row justify-between items-center">
                <a-input-search allowClear v-model:value="keyword" placeholder="搜索" enter-button size="middle"
                    style="width: calc(var(--spacing) * 70)" @search="searchAccountLibrary(keyword)" />
            ...
            <div ref="scrollArea" class="flex-1 overflow-y-auto overflow-x-hidden">
                <transition :name="transitionDirection ? 'slide-' + transitionDirection : ''" mode="out-in">
                    <div :key="viewKey" class="h-full">
                        <div v-if="localRadio === 'grid' && librarys.length !== 0"
                            ...
                            <template v-if="librarysLoading">
                                <div v-for="i in 8" :key="i" class="w-full h-55 rounded-lg bg-gray-200 animate-pulse">
                                </div>
                            </template>
                            <template v-else>
                                <div v-for="(lib, index) in librarys" :key="index" class="relative w-full h-55 group">
                                    <div
                                    ...
                                    <a-dropdown :trigger="['contextmenu']">
                                        <a-card hoverable @click="routeToGameDetail(lib)"
                                            ...
                                            <div class="absolute inset-0">
                                                <img :src="`load://${useSteamStore().steam.installPath.replace(/\\/g, '/')}${lib.appPicture}`"
                                                    class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
                                                    alt="游戏封面图" />
                                            </div>
                                            <div class="
                                                    absolute bottom-0 left-1/2 -translate-x-1/2
                                                    w-4/5 max-w-100 px-2 py-1
                                                    bg-black/60 backdrop-blur-sm
                                                    text-white text-xs text-center rounded-t-md
                                ...
                                <template #renderItem="{ item, index }">
                                    <a-list-item>
                                        <div class="flex w-full">
                                            ...
                                            <template v-else>
                                                <img width="120" alt="logo" class="object-cover rounded flex-shrink-0"
                                                    :src="`load://${useSteamStore().steam.installPath.replace(/\\/g, '/')}${item.appPicture}`" />
                                            </template>
                                            <div class="flex-1 flex flex-col ml-4">
                                                <div class="text-lg font-semibold mb-1">
                                                    <a-skeleton v-if="librarysLoading" :title="false"
                                                        :paragraph="{ rows: 1, width: 120 }" active />
                                                    <template v-else>
                                                        {{ item.appName }}
                                                    </template>
                                                </div>
                                                <div class="text-sm text-gray-600 leading-tight">
                                                    <a-skeleton v-if="librarysLoading" :title="false"
                                                        :paragraph="{ rows: 1, width: 200 }" active />
                                                    <template v-else>
                                                        {{ item.appLocalized }}
                                                    </template>
                                                </div>
                                                <div class="flex justify-end items-end flex-1">
                                                    <a-skeleton v-if="librarysLoading" :title="false"
                                                        :paragraph="{ rows: 1, width: 100 }" active />
                        ...
                        <div v-if="librarys.length === 0 && (localRadio === 'list' || localRadio === 'grid')" class="overflow-y-hidden">
                            <a-empty :image="emptyIcon" :image-style="{
                                display: 'flex',
...
.slide-right-leave-active {
    transition: all 0.20s cubic-bezier(0.55, 0, 0.1, 1);
}
