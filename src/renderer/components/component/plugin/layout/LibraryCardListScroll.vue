<script setup lang="ts">
  ...
  import LibraryGameListCard from '../card/LibraryGameListCard.vue';
...
<template>
  <!-- 骨架屏加载 -->
  <div v-if="loading" class="px-4 py-2 flex flex-col gap-3">
    <div v-for="i in 8" :key="i" class="w-full h-20 bg-gray-200 rounded-lg animate-pulse"></div>
  </div>
  <div v-else>
    <!-- 非空LibraryDetail结果显示 -->
    <div v-if="library.length !== 0" class="px-4 py-2 flex flex-col gap-3">
      <LibraryGameListCard
