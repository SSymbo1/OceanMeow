...
import { ipcHandlerRegister, protocolRegister } from '#/register';
...
if (process.platform === 'win32') {
  try {
    if (app.isPackaged) {
      const squirrelPath = path.join(process.resourcesPath, 'node_modules', 'electron-squirrel-startup');
...
const createWindow = () => {
  const mainWindow = new BrowserWindow({
    ...
    thickFrame: true,
    icon: app.isPackaged ? path.join(process.resourcesPath, 'icon.ico') : path.join(__dirname, '../public/icon.ico'),
    webPreferences: {
