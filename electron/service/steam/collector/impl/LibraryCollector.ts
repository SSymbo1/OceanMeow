...
import { Library, LibraryTime, SteamAccount } from "#/entity";
...
export class LibraryCollector implements SteamDataCollector<Library> {
    ...
    */
    async collect(steamInstallPath: string): Promise<Library[]> {
        ...
        const accounts = await SystemDB.getInstance().typeROM.getRepository(SteamAccount).find({
            select: ['steamId']
        });
        const localConfigAppId = new Set<string>();
        let localConfigAppTime: LibraryTime[] = [];
        let librarys: Library[] = [];
        for (const account of accounts) {
            const localConfigPath = join(steamInstallPath, Steam.LOCAL_CONFIG_VDF.replace('{user_id}', account.steamId));
        ...
        const libraryTimeResult = await libraryTimeRepo.save(localConfigAppTime);
        ...
        if (appInfoVDF !== null) {
            for (const app of Array.from(localConfigAppId)) {
                ...
                librarys.push(libraryRepo.create({
                    appId: app,
                    name: common.name || '',
                    nameLocal:
